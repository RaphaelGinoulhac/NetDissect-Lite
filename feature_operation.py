...
class FeatureOperator:
    ...
    def feature_extraction(self, model=None, memmap=True):
        ...
        if memmap:
            ...
            mmap_max_files = [os.path.join(settings.OUTPUT_FOLDER, "%s_max.mmap" % feature_name) for feature_name in
                              settings.FEATURE_NAMES]
        ...
        for batch_idx, batch in enumerate(loader.tensor_batches(bgr_mean=self.mean)):
            # modified, testing the is None condition here rather than in the generator, which would raise an exception and terminate the program
            if batch is None:
                break
            del features_blobs[:]
            input = batch[0]
    ...
    def tally(self, features, threshold, savepath=''):
        ...
        if settings.PARALLEL > 1:
            ...
            ranges = [(s, min(self.data.size(), s + psize)) for s in range(0, self.data.size(), psize) if
                      s < self.data.size()]
            params = [(features, self.data, threshold, tally_labels, tally_units, tally_units_cat, tally_both) + r for r
                      in ranges]
            threadpool = pool.ThreadPool(processes=settings.PARALLEL)
            threadpool.map(FeatureOperator.tally_job, params)
        else:
            FeatureOperator.tally_job((features, self.data, threshold, tally_labels, tally_units, tally_units_cat,
                                       tally_both, 0, self.data.size()))
        ...
        pciou = np.array([iou * (primary_categories[np.arange(iou.shape[1])] == ci)[np.newaxis, :] for ci in
                          range(len(self.data.category_names()))])
